...
const PrintPOAttachment = () => {
  const [po, setPO] = useState(null);

  useEffect(() => {
    const saved = localStorage.getItem('print_po_data');
    if (saved) setPO(JSON.parse(saved));
  }, []);
  ...
  vendorInvPhotos.forEach((p, idx) => {
    if (p) docs.push({ src: p, label: 'INVOICE VENDOR' });
  });

  // Payment Proofs
  const paymentPhotos = getPhotos(po.paymentProofPhoto);
  paymentPhotos.forEach((p, idx) => {
    if (p) docs.push({ src: p, label: 'BUKTI PEMBAYARAN (PAYMENT PROOF)' });
  });

  // Tax Proofs
  const taxPhotos = getPhotos(po.tax_proof_photo);
  taxPhotos.forEach((p, idx) => {
    if (p) docs.push({ src: p, label: 'BUKTI POTONG PAJAK (TAX PROOF)' });
  });
  ...
  if (allPhotos.length === 0) return (
    <div style={{ padding: '60px', textAlign: 'center', fontFamily: 'Inter, sans-serif', color: '#94a3b8' }}>
      ...
      <p style={{ fontWeight: '700', fontSize: '1.1rem' }}>Tidak ada foto dokumentasi untuk PO ini.</p>
    </div>
  );
        ...
        >
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '3px solid #1e293b', paddingBottom: '18px', marginBottom: '28px' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: '1rem', fontWeight: '900', color: '#64748b', textTransform: 'uppercase', letterSpacing: '1px' }}>{p.label}</div>
              <div style={{ fontSize: '0.75rem', fontWeight: '800', color: '#1e293b', marginTop: '4px' }}>PO ID: {po.id}</div>
            </div>
          </div>
          ...
          <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: '1px dashed #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
            <p style={{ fontSize: '0.65rem', color: '#94a3b8', margin: 0, fontWeight: '600' }}>
              Halaman {i + 1} dari {allPhotos.length} — Dokumen lampiran pelunasan vendor.
            </p>
            <div style={{ textAlign: 'center', minWidth: '180px' }}>
              <div style={{ borderBottom: '1px solid #1e293b', width: '100%', marginBottom: '6px' }}></div>
              <p style={{ margin: 0, fontWeight: '800', fontSize: '0.8rem', color: '#1e293b' }}>Accounting Division</p>
            </div>
          </div>
        </div>
      ))}
